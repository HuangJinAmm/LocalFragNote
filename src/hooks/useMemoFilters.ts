...
export const useMemoFilters = (options: UseMemoFiltersOptions = {}): string | undefined => {
  ...
  return useMemo(() => {
    ...
    // 收集 contentSearch、semanticSearch、tagSearch，聚合后生成条件
    const contentSearchValues: string[] = [];
    const semanticSearchValues: string[] = [];
    const tagSearchValues: string[] = [];
    for (const filter of filters) {
      if (filter.factor === "contentSearch") {
        contentSearchValues.push(filter.value);
      } else if (filter.factor === "semanticSearch") {
        semanticSearchValues.push(filter.value);
    ...
    if (contentSearchValues.length > 0) {
      const query = contentSearchValues.join(" ");
      conditions.push(`fts.match(${escapeFilterValue(query)})`);
    }
    // semanticSearch → 单个 semantic.search（取最后一个有效值）
    if (semanticSearchValues.length > 0) {
      const query = semanticSearchValues[semanticSearchValues.length - 1];
      conditions.push(`semantic.search(${escapeFilterValue(query)})`);
    }
    // 聚合所有 tagSearch 为单个 tag in [...] 条件
    if (tagSearchValues.length > 0) {
      const tagList = tagSearchValues.map((v) => escapeFilterValue(v)).join(", ");
      conditions.push(`tag in [${tagList}]`);
    }
