...
// Tauri 自定义协议：通过 attachment:// 协议访问本地附件 blob
// Windows 上实际 URL 为 http://attachment.localhost/...
const ATTACHMENT_BASE = "http://attachment.localhost";

export const getAttachmentUrl = (attachment: Attachment) => {
  if (attachment.externalLink) {
    return attachment.externalLink;
  }
  return `${ATTACHMENT_BASE}/${attachment.name}`;
};

export const getAttachmentThumbnailUrl = (attachment: Attachment) => {
  return `${ATTACHMENT_BASE}/${attachment.name}?thumbnail=true`;
};

export const getAttachmentMotionClipUrl = (attachment: Attachment) => {
  return `${ATTACHMENT_BASE}/${attachment.name}?motion=true`;
};
